-- #############################################################################
-- НЕ ПУСКАЙ. Заменен от каноничния слой на филтрите (V38–V41, FilterIndexService,
-- scripts/55_filters_curated_*), 2026-09-25.
-- Скриптът пипа суровия слой (parameters / parameter_options /
-- category_parameters.is_filter), който sync-овете владеят и пренаписват всяка
-- нощ; магазинът вече не чете is_filter. Пазим го само за история.
-- Защитата отдолу отваря транзакция и я проваля, така че нищо след нея не се
-- записва, дори при изпълнение, което продължава след грешка.
-- #############################################################################
BEGIN;
DO $$ BEGIN
  RAISE EXCEPTION 'Скрипт 40a е заменен от каноничния слой на филтрите — НЕ ПУСКАЙ';
END $$;
