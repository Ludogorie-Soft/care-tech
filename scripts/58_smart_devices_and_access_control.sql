...
-- ── 1. Продукти с ясна видима категория ──────────────────────────────────────
CREATE TEMP TABLE move_58 (product_id BIGINT PRIMARY KEY, from_category BIGINT NOT NULL, to_category BIGINT NOT NULL)
    ON COMMIT DROP;
INSERT INTO move_58 (product_id, from_category, to_category) VALUES
    ...
    (27509, 325, 500), (27512, 325, 500);                      -- Realme кантар и крушка → Смарт устройства
...
UPDATE categories
SET parent_id     = 230,
    name_bg       = 'Контрол на достъп',
    name_en       = 'Access control',
    slug          = 'kontrol-na-dostap',
    category_path = 'videonablyudenie/kontrol-na-dostap',
    show_flag     = TRUE,
    sort_order    = 7,
    updated_at    = NOW()
WHERE id = 548;

-- ── 3. Смарт устройства — видим корен ────────────────────────────────────────
...
UPDATE products
SET manually_hidden = FALSE,
    show_flag       = (active AND status = 'AVAILABLE' AND NOT deleted),
    updated_at      = NOW()
WHERE id IN (19016, 19170, 11408, 11413, 27509, 27512) AND manually_hidden;

DO $$
DECLARE moved INT; back INT;
BEGIN
    SELECT count(*) INTO moved FROM move_58 m JOIN products p ON p.id = m.product_id AND p.category_id = m.to_category;
    SELECT count(*) INTO back FROM products WHERE id IN (19016, 19170, 11408, 11413, 27509, 27512) AND NOT manually_hidden;
    IF moved <> 13 THEN
        RAISE EXCEPTION 'ПРЕКЪСНАТО: преместени са % от 13 продукта — някой не е там, където го очаквах.', moved;
    END IF;
...
-- Контрола (след COMMIT, само чете).
