...
- **ASBIS продуктовият sync разпознава категорията на нов продукт САМО по името на ВИДИМА коренова категория** (productcategory) и по видимо дете (producttype) под нея. Преименуване или преместване под друг родител спира автоматичното внасяне на новите продукти (съществуващите остават). Затова 500 остава „Смарт устройства“ (коренова). Ubiquiti Access идва като „Other / Networking - Access Control“ — не се внася автоматично нито преди, нито след скрипт 58.
- **Нова видима коренова категория иска sort_order (0 = най-горе в менюто) и икона по slug в care-tech-ui `src/utils/navIcons.js`** (иначе FaRegFolder).
- **[2026-09-25] Решение: смарт дом (Aqara, IMOU) и контрол на достъп (Ubiquiti) СЕ ПРЕДЛАГАТ** — видими категории 500 и 548 (скрипт 58). Бялата техника остава спряна (скриптове 50, 57).
- **[2026-09-25] Потребителят пуска скриптовете и през DBeaver (грешки „SQL Error [25P03]“).** Многокомандна транзакция + idle_in_transaction_session_timeout 60 s се прекъсва, ако DBeaver спре на диалог — и истинската грешка се губи. За скриптове, които пишат: целият запис в ЕДИН DO блок (атомарен, без BEGIN/COMMIT), броячи с GET DIAGNOSTICS, контролни SELECT-и отделно след него.
- **DBeaver SQL редакторът на потребителя е в Manual commit** (видяно в pg_stat_activity: „DBeaver … SQLEditor <Script-NN.sql> — idle in transaction“). С нашия header (idle_in_transaction_session_timeout 60 s) записът се отменя, ако няма commit. Затова скриптовете, които пишат, завършват с `COMMIT;` СЛЕД последния контролен SELECT — работи и в ръчен, и в auto-commit режим. Проверка дали скрипт е приложен: pg_stat_activity + самите данни, не съобщението в клиента.
